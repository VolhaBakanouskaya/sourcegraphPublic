...
public class CodyCompletionsManager {
  ...
  public void clearCompletions(Editor editor) {
    this.currentJob.get().ifPresent(j -> j.cancel(true));
    for (Inlay<?> inlay :
  ...
  public void triggerCompletion(Editor editor, int offset) {
    ...
    if (this.currentJob.get().isPresent()) {
      this.currentJob.get().ifPresent(j -> j.cancel(true));
    }
    this.currentJob.set(Optional.of(this.scheduler.schedule(callable, 500, TimeUnit.MILLISECONDS)));
  }
